...
function zoomIndexScale(scale, zoom, center, zoomOptions) {
	...
	var scaleIndex = Math.floor((scale.maxIndex - scale.minIndex) / 5);
	...
	if (Math.abs(zoomNS.zoomCumulativeDelta) > sensitivity){
		if(zoomNS.zoomCumulativeDelta < 0){
			if(centerPointer >= chartCenter){
				if (minIndex <= 0){
					maxIndex = Math.min(lastLabelIndex, maxIndex + scaleIndex);
				} else{
					minIndex = Math.max(0, minIndex - scaleIndex);
				}
			...
			if(centerPointer >= chartCenter){
				minIndex = minIndex < maxIndex ? minIndex = Math.min(maxIndex, minIndex + scaleIndex) : minIndex;
			} else if(centerPointer < chartCenter){
				maxIndex = maxIndex > minIndex ? maxIndex = Math.max(minIndex, maxIndex - scaleIndex) : maxIndex;
			}
			zoomNS.zoomCumulativeDelta = 0;
		}
